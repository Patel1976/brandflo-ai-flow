...
export default function ContentCalendar() {
  ...
  const getPostsForDate = (date: Date | number) => {
    let dateStr = "";
    if (typeof date === "number") {
      dateStr = `2024-01-${date.toString().padStart(2, "0")}`;
    } else {
      const year = date.getFullYear();
      ...
      <div className="p-6 space-y-6">
        ...
        <Card className="border-card-border">
          <CardContent className="p-0">
            {/* Days of week */}
            <div className="grid grid-cols-7 border-b border-card-border text-xs sm:text-sm">
              {dayNames.map((day) => (
                <div
                  key={day}
                  className="p-2 sm:p-4 text-center font-medium text-muted-foreground border-r border-card-border last:border-r-0"
                >
                  {day}
                </div>
              ))}
            ...
            {viewMode === "month" ? (
              <div className="grid grid-cols-7 text-xs sm:text-sm">
                {getDaysInMonth(currentDate).map((day, index) => (
                  <div
                    key={index}
                    className="min-h-24 sm:min-h-32 border-r border-b border-card-border last:border-r-0 p-1 sm:p-2"
                  >
                    {day && (
                      <div className="space-y-1 sm:space-y-2">
                        ...
                        <div className="space-y-1">
                          {getPostsForDate(day).map((post) => (
                            <div
                              key={post.id}
                              className="p-1 sm:p-2 bg-primary/5 rounded text-[10px] sm:text-xs cursor-pointer hover:bg-primary/10"
              ...
              <div className="grid grid-cols-7 text-xs sm:text-sm">
                {getDaysInWeek(currentDate).map((day, index) => (
                  <div
                    key={index}
                    className="min-h-32 border-r border-b border-card-border last:border-r-0 p-1 sm:p-2"
                  >
                    <div className="space-y-1 sm:space-y-2">
          ...
          <Card className="border-card-border">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                {isEditing ? (
                  <Textarea value={selectedPost.content} className="mt-1" rows={3} />
                ) : (
